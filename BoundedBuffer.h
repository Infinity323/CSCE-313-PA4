...
private:
	...
	mutex mtx;
	condition_variable cv_over, cv_under; // over for full queue, under for empty queue
...
public:
	...
	void push(char* data, int len) {
		...
		cv_over.wait(lck, [this]{ return q.size() < cap; });
	...
	int pop(char* buf, int bufcap) {
		...
		unique_lock<mutex> lck(mtx);
		cv_under.wait(lck, [this]{ return q.size() > 0; });
